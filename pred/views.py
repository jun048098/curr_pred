...
def result(request):
    name = request.POST['currname']
    num = request.POST['currnum']


    if num == '':
        is_exist = False
        pred = curr_predict(name)


    else:
        num = int(num)
        is_exist = True
        pred = curr_predict(name, num)

    return render(request, 'pred/result.html', {'currname': name, 'is_exist':is_exist , 'pred':pred, num:'num'})
